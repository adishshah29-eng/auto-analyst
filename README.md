...
| Critic findings review | Real catches on findings the significance gate has no opinion on: dropped a finding restating a basic row/survival-rate summary and two near-zero correlations explicitly flagged as noise (titanic_like, 2 kept of 5). Caveated findings are a separate, non-negotiable path (see below): on leads_deals, 3 of 4 findings were caveated by the gate and none were droppable — all 4 kept, all 3 caveats hedged in the narrative |
...
**A caveated finding is never dropped by the Critic's LLM pass — enforced in code, not left to the model's judgment.** The first version of this asked the Critic to "weigh the caveat rather than treat it as automatic grounds to drop," and the exact same code produced two different outcomes on two live runs of structurally different datasets: once the Critic dropped three small-sample findings outright (narrative: *"the analysis can't answer which region... because findings only provide an overall baseline"*), once it kept an equivalent set and the Synthesizer hedged each one in place (*"though this win rate rests on a small subgroup and should be treated as exploratory"*). Both are individually defensible; having the exact same input produce either one at random is not. Every caveat in this codebase comes from the same deterministic check and means the same thing — real number, sample/effect size too weak to state as firm — so `review_findings()` now force-restores any caveated finding's index into `keep_indices` after the LLM call, regardless of what the LLM decided or why. The Critic's LLM pass still fully controls everything the gate has no opinion on (trivial restatements, a description that misstates its own stats, duplicates). Re-verified live on the same churn-dataset question that used to produce a refusal: the narrative now opens with *"LatAm has the worst churn rate at 0.0833, and Partner is the worst acquisition channel at 0.0800, though both figures rely on small subgroups and should be treated as exploratory"* — directly answering the question asked, with the caveat attached to every number, every time.
...
**A prompt instruction that was ambiguous enough to produce two opposite, individually-reasonable outcomes from the identical code.** After the significance gate above was fixed and verified, the Critic's prompt still told the LLM to "weigh a caveat rather than treat it as automatic grounds to drop" — a judgment call, not a rule. Run against one dataset, the Critic dropped every caveated finding and the narrative refused to answer the question asked. Run against a different dataset, it kept an equivalent set and the Synthesizer hedged them in place instead. Neither is wrong on its own; a system that flips between them on the same class of input, for no visible reason, is untrustworthy in a specific way that's easy to miss because every individual run still *looks* fine — "it answered" and "it declined to answer, correctly noting why" both read as reasonable outputs in isolation. The tell was only visible by comparing two separate live runs against different datasets. Fixed by moving the actual policy decision out of the prompt and into code: a caveated finding's index is force-restored into `keep_indices` after the LLM call, so the Critic's LLM pass no longer has the *option* to drop one for that reason, whatever it decides. The prompt still explains the rule (so the model's own `drop_reasons` stay coherent), but the rule no longer depends on the model reading it correctly.
